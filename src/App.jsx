...
import { getDatabase, ref, set, onValue, runTransaction, remove } from "firebase/database";
...
function exportToPDF(sessionName, cards) {
  ...
  let html = `
    ...
    <style>
      ...
      .card-author { font-size: 11px; color: #94a3b8; margin: 0 0 6px 0; }
      .reactions { font-size: 12px; color: #64748b; }
  ...
  COLUMNS.forEach(col => {
    ...
    if (colCards.length === 0) {
      ...
      colCards.forEach(c => {
        const reactionSummary = Object.entries(c.reactions || {})
        ...
        html += `<div class="card">
          ...
          ${reactionSummary ? `<p class="reactions">${reactionSummary}</p>` : ""}
        </div>`;
      });
    }
    html += `</div></div>`;
...
export default function App() {
  ...
  const submitCard = useCallback(async () => {
    ...
    const card = { col: cardCol, text: cardText.trim(), author: userName, reactions: {} };
  ...
  const toggleReaction = async (cardId, emoji) => {
    ...
    const reactionRef = ref(db, `sessions/${sessionId}/cards/${cardId}/reactions/${safeEmoji}/${userName}`);
    await runTransaction(reactionRef, cur => cur ? null : true);
  };
                ...
                {displayCards.map(card => {
                  ...
                  return (
                    <div key={card.id} className="bg-white rounded-lg px-3 py-2 shadow-sm text-sm border border-slate-200 group relative">
                      ...
                      <p className="text-xs text-slate-400 mt-1 mb-2">{card.author === userName ? "You" : card.author}</p>
                      {canSeeAll && (
                        <div className="flex flex-wrap gap-1">
                          {EMOJIS.map(emoji => {
                            const safeEmoji = encodeURIComponent(emoji);
                            const users = reactions[safeEmoji] || {};
                            ...
                            return (
                              <button key={emoji} onClick={() => toggleReaction(card.id, emoji)}
                                className={`flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded-full border transition
                                  ${reacted ? "bg-indigo-100 border-indigo-300 text-indigo-700" : "bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100"}`}>
                                <span>{emoji}</span>
